...
I have described \calculusName{}, a calculus for studying the interaction of effects and metaprogramming. This chapter illustrates how \calculusName{} can be used to precisely define scope extrusion, and evaluate scope extrusion solutions. I describe and evaluate four solutions: 
\begin{enumerate}
...
Finally, drawing on my use of \calculusName{}, I briefly evaluate \calculusName{}'s ability to facilitate comparative evaluation of scope extrusion solutions (\Cref{section:evaluation}).
...
I now define the lazy dynamic check precisely as a modified term elaboration ${\elaborate{}}^{\textbf{Lazy}}$. I need to make two changes: first, I insert \textbf{\texttt{dlet}}s into \compilemode{} cases, for example
{
  \footnotesize
...
It is possible to define an eager dynamic check by extending the lazy dynamic check. In addition to the top-level splice check, and the \compilemode{}-mode \textbf{\texttt{dlet}}s, I add \textbf{\texttt{dlet}}s and \textbf{\texttt{check}}s in \quotemode{}-mode, for example 
{
  \footnotesize
\[
\begin{array}{rcl}
...
Intuitively, we insert checks whenever we build ASTs. Hence, assume that we reduce to a configuration that exhibits eager scope extrusion. Let the offending AST be $n$. The error will be detected and reported the next time $n$ is used to build a bigger AST in an unsafe way. 

As an example, consider the following \sourceLang{} program in \Cref{listing:eager-scope-extrusion-check-eg}. Eager scope extrusion is caused by the $\return{y}$ program fragment, where $y$ refers to the unbound variable $\Var{x}{\mathbb{N}}$\footnote{Technically, the variable should be renamed. For clarity, I do not rename the variables in this example}. Eager scope extrusion is not caught immediately. Rather, $y$ is bound to $z$, and scope extrusion is caught when $z$ is used to build a larger AST, $\equote[{\splice[z]+1}].$ 
...
We can attempt to prove the correctness of the eager dynamic check: all cases of eager scope extrusion are reported (by transitioning to \textbf{\texttt{err}}). This eager dynamic is, unfortunately, incorrect: we may cause eager scope extrusion, but never use the offending AST $n$ in an unsafe way. 

For example, recall that we can write non-terminating programs in \sourceLang{}. Consider the \sourceLang{} program in \Cref{listing:eager-scope-extrusion-unsafe}, where $\Omega$ is some non-terminating program that never refers to $z$. After translation, the program will reduce to a configuration that exhibits eager scope extrusion ($\return{y}$). However, the program will immediately enter into a non-terminating loop, and scope extrusion will never be reported.
...
\begin{code} 
 \begin{source}
  $\begin{array}{l}
    ...
    \end{array}$
 \end{source}
 \captionof{listing}{The eager dynamic check does not report all occurrences of eager scope extrusion. In the program, we cause eager scope extrusion ($\return{y}$), and then enter into a non-terminating loop which never refers to $y$.}%
...
\begin{code} 
 \begin{source}
  $\begin{array}{l}
    ...
    \end{array}$
 \end{source}
 \captionof{listing}{The eager dynamic check additionally will not report eager scope extrusion in the case where the offending AST is thrown away.}%
...
In \Cref{listing:eager-scope-extrusion-looks-unsafe}, the offending AST ($\Var{x}{\mathbb{N}}$, bound to $y$) is used in a way that appears unsafe ($\equote[{\splice[y]} + 0]$, bound to $w$). The program fragment $\equote[{\splice[y]} + 0]$ elaborates into \[\checkfv{\texttt{Plus}(\Var{x}{\mathbb{N}}, \texttt{Nat}(0))}\] 
...
I will build this check incrementally. Consider, again, the program in \Cref{listing:eager-scope-extrusion-looks-unsafe}, which I elaborate it into \coreLang{} \footnote{The program has been somewhat simplified}. I have \colorwave[quote]{underlined} the failing check. 
\begin{center}
  \begin{tikzpicture}
...
\end{array}\]
Note that the body of $f$, coloured in \textbf{\textcolor{comment}{grey}}, is the simplified version of \Cref{listing:eager-scope-extrusion-looks-unsafe}. The \textbf{\texttt{extrude}} operation is caught by the handler (also in \textbf{\textcolor{comment}{grey}}), and never captures the surrounding context (in \textbf{black}). This surrounding context, which must have no references to the captured continuation, $k$, is identified by $I$.
...
Rather than expressiveness, I prove a ``cause-for-concern'' property. Effectively, it states that if the best-effort check reports an error, \textbf{and the offending AST $n$ appears in the final result of compile-time computation}, then there must be lazy scope extrusion. Consequently, in cases like \Cref{listing:best-effort-imperfect}, the only way to safely use $\lambda x.\return{y}$ is to throw it away. 
...
The logical relation on terms is defined as a least fixed point, following the definitions by \citet{plotkin-2025} and \citet{kuchta-2023}, where the well-foundedness of the definition is additionally justified. I rely on the following induction principle, which I call \textsf{Scoped}-Induction
...
\begin{enumerate}
  \item If $\elaborate{e}_{\compilemode}({\rho, \Binder{\alpha}{T_1^{\gamma'}} / x}) \in \scoped[\Theta']{\effectType[\Delta; \xi]{T_2^{0}} (\gamma')}$ reduces to some $\return{n}$, then by the inductive hypothesis it is of AST type and all its free variables are permitted by $\gamma'$. The term $\bind{\texttt{body}}{\return{n}}{\Lam{\Binder{\alpha}{T_1^{\gamma'}}}{\texttt{body}}}$ reduces to $\Lam{\Binder{\alpha}{T_1^{\gamma'}}}{n}$, where $\Var{\alpha}{T_1^{\gamma'}}$ is bound. By the typing rules, of the free variables, $\alpha$ is the only variable tagged with classifier $\gamma'$. So we can conclude, under $\Theta$, that the free variables of $\Lam{\Binder{\alpha}{T_1^{\gamma'}}}{n}$ are permitted by $\gamma$. The  conclusion thus follows from anti-reduction.
