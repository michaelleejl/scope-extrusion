...
\end{enumerate}
I have implemented the three dynamic checks in MacoCaml, with the best-effort check merged into MacoCaml. The implementations closely mirror the descriptions in this chapter, and were useful for building an understanding of the properties of the checks. 
...
Second, \textbf{\texttt{dlet}}s are inserted to ensure variables bound outside top-level splices (in \compilemode{}-mode) are declared safe in the context surrounding the top-level splice (\Cref{dfn:declared-safe}). Variables bound outside top-level splices thus do not cause the \textbf{\texttt{check}} to fail. For example, in $\lambda x:\mathbb{N}. \; \splice[{\equote[\lambda{y}: \mathbb{N}. \; x+y]}]$, at the point of the check, $x$ is declared safe, and thus allowed to be free, but $y$ is not. Elaboration of formal parameters in \compilemode{}-mode (but not \quotemode{}-mode) should insert \textbf{\texttt{dlet}}s:
{
  \footnotesize
...
A configuration exhibits \textbf{eager scope extrusion} if it is improperly scoped (\Cref{subsubsection:eager-dynamic-check}, \Cpageref{subsubsection:eager-dynamic-check}) at any point in the execution. \Cref{dfn:eager-scope-extrusion} thus generalises \Cref{def:lazy-scope-extrusion}. 
...
If \textbf{\texttt{dlet}}s were not inserted for binders in \quotemode{}-mode (e.g. $y$), then $\textbf{\texttt{check}}\; \texttt{Plus}(a, b)$ would fail, as $y$ would not be declared safe at the point of the check. 
...
The eager dynamic check is not, in the worst case, more efficient than the lazy dynamic check, since there exist pathological examples, such as when the offending AST is not used to construct a larger AST, but returned at the top-level splice (\Cref{listing:eager-scope-extrusion-check-eg-tls}). However, I hypothesise that in the \textit{common case}, the eager dynamic check detects more efficiently. This question is an empirical one, and would require further research, outside the scope of the dissertation, to resolve. 
...
\begin{center}
  ...
  \begin{scope}[every node/.style={font=\scriptsize}]
    \node[anchor = north, text=quote, align = left] (check-fail) at ($(example.south) + (1, -0.2)$) {\textbf{\texttt{check}} fails, transitioning to $\err$, since \\ $\Var{x}{\mathbb{N}}$ is bound to $y$, and $\Var{x}{\mathbb{N}}$ is\\ not declared safe via \textbf{\texttt{dlet}}};
...
The check fails because when $\textbf{\texttt{extrude}}$ performed, the variable $\Var{x}{\mathbb{N}}$ is no longer declared safe in the new evaluation context. Since $\Var{x} {\mathbb{N}}$ is bound to $y$, checking $\texttt{Plus}(y, \texttt{Nat}(\texttt{0}))$ reports an error. The problem is that the continuation $k$ can be used to bind $\Var{x}{\mathbb{N}}$. It is not clear, when the \texttt{Plus} AST is constructed and checked, that eager scope extrusion \textit{must} lead to lazy scope extrusion. 
...
\begin{enumerate}
  \item Elaboration of types erases classifiers. For example,  $\elaborate{(\textsf{Code}(\mathbb{N}^{0})^{\gamma})^{-1}} = \mathbb{\textsf{AST}(\mathbb{N})}$.
...
{\scriptsize{\textit{Given $\effectType{\tau}$, $\tau$ refers to the corresponding compile-time value type. For example, if $\effectType{\tau} = \effectType[\Delta; \xi]{T^{0}} (\gamma)$, then $\tau = \effectType[\xi]{T^{0}} (\gamma)$}}}\\
