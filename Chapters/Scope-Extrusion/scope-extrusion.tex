...
We can attempt to prove the correctness of the eager dynamic check: all cases of eager scope extrusion are reported (by transitioning to \textbf{\texttt{err}}). This eager dynamic is, unfortunately, incorrect: we may cause eager scope extrusion, but never use the offending AST $n$ in an unsafe way, and thus the eager dynamic check will not report that scope extrusion occurred. 
...
\begin{lemma}[Weakening for Level $-1$ Values]{sourceHighlight}
  ...
  \begin{enumerate}
    \item $\stypejudge[\Gamma, (x:T_1^0)^{\gamma'}]{e}{T^0}$ for arbitrary $\gamma' \in \Gamma$
