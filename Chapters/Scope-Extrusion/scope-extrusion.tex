...
This chapter illustrates how \calculusName{} can be used to precisely define scope extrusion, and evaluate scope extrusion checks. It describes and evaluates four checks: 
\begin{enumerate}
...
\end{enumerate}
The three dynamic checks have additionally been implemented in MacoCaml. The implementations closely mirror the descriptions in this chapter, and were useful for building an understanding of the correctness and expressiveness of the various checks. 

Finally, \Cref{section:evaluation} reflects on the use of \calculusName{}, assessing its ability to facilitate comparative evaluation of scope extrusion checks.
...
Note that the marker for top-level splice \textbf{\texttt{tls}} implies that $n$ is spliced into a larger, ambient and inert, program. 

The lazy dynamic check is defined as a modified term elaboration ${\elaborate{}}^{\textbf{Lazy}}$. Two modifications are made to the term elaboration described in \Cref{section:elaboration}. First, checks are performed after top-level splices (I write $\checkfv{e}$ for $\bind{x}{e}{\checkfv{x}}$):
\[\elaborate{\splice[e]}_{\compilemode{}}^{\textbf{Lazy}} \triangleq {\checkfv{(\tls{\elaborate{e}_{\splicemode{}}^{\textbf{Lazy}}})}}\]
Second, to ensure variables bound outside top-level splices do not cause the \textbf{\texttt{check}} to fail. For example, in $\lambda x:\mathbb{N}. \; \splice[{\equote[\lambda{y}: \mathbb{N}. \; x+y]}]$, $x$ is in \compilemode{}-mode while $y$ is in \quotemode{}-mode. $x$ should be declared safe (should be allowed to be free, and not cause a transition to \textbf{\texttt{err}}), but $y$ should not. Thus, elaboration of binders (like $x$) in \compilemode{}-mode (but not \quotemode{}-mode) should be declared via \textbf{\texttt{dlet}}:
{
  \footnotesize
...
Consequently, to prevent false positives, the eager dynamic check also needs to insert \textbf{\texttt{dlet}}s for binders in \quotemode{}-mode, not just \compilemode{}-mode:
{
  \footnotesize
\[
\begin{array}{rcl}
...
Note that if \textbf{\texttt{dlet}}s were not inserted for binders in \quotemode{}-mode, then $\textbf{\texttt{check}}\; \texttt{Plus}(a, b)$ would fail, as $y$ would not be declared safe. 

Intuitively, the eager dynamic check performs a check whenever an AST is built. Hence, assume that evaluation reduces to a configuration that exhibits eager scope extrusion. Let the offending AST be $n$. The error will be detected and reported the next time $n$ is used to build a bigger AST in an unsafe way (not all free variables in $n$ declared safe via \textbf{\texttt{dlet}}). 
...
Any attempt to prove the correctness of the eager dynamic check: all cases of eager scope extrusion are reported (by transitioning to \textbf{\texttt{err}}) will fail. The eager dynamic check is incorrect, since evaluation may result in eager scope extrusion, but the offending AST $n$ may never be used in an unsafe way. Thus, the eager dynamic check will not report that scope extrusion occurred. 
...
\begin{code} 
 \begin{source}
  $\begin{array}{l}
    ...
    \end{array}$
 \end{source}
 \captionof{listing}{The eager dynamic check does not report all occurrences of eager scope extrusion. The program causes eager scope extrusion ($\return{y}$), and then enters into a non-terminating loop which never refers to $y$.}%
...
The expressivenss of the eager dynamic check, however, is not without issue. First, the eager dynamic check reports false positives (\Cref{listing:eager-scope-extrusion-looks-unsafe}), and is thus less expressive than the lazy dynamic check, with  illustrating an example.
...
\begin{code} 
 \begin{source}
  $\begin{array}{l}
    ...
    \end{array}$
 \end{source}
 \captionof{listing}{A program which will fail the eager dynamic check. The offending AST ($\Var{x}{\mathbb{N}}$) is used to construct a larger AST in a way that appears to be unsafe $\equote[{\splice[y]+0}]$, but is actually not, since the unsafe AST is then only used in a safe way.}%
...
and the \textbf{\texttt{check}} fails because $\Var{x}{\mathbb{N}}$ is free and unbound. However, similar to \Cref{listing:eager-scope-extrusion-unsafe-continue}, the continuation containing $\dlet{\Binder{x}{\mathbb{N}}}{[-]}$ is restored, and $\texttt{Plus}(\Var{x}{\mathbb{N}}, \texttt{Nat}(0))$ will be properly scoped. The program \textit{recovers} from a state of eager scope extrusion, and will not exhibit lazy scope extrusion. The eager dynamic check is thus not as expressive as the lazy dynamic check.
...
The best-effort dynamic check may be built incrementally. Consider, again, the program in \Cref{listing:eager-scope-extrusion-looks-unsafe}, which is elaborated into \coreLang{} \footnote{The program has been somewhat simplified}. The failing check is \colorwave[quote]{underlined}. 
\begin{center}
  \begin{tikzpicture}
...
The check fails because when an effect is performed, the variable $\Var{x}{\mathbb{N}}$ is no longer declared safe ($\dlet{\Binder{x}{\mathbb{N}}}{[-]}$ no longer on the stack). Since $y$ is bound to $\Var{x}{\mathbb{N}}$, executing the handler for \textbf{\texttt{extrude}} reports an error. The problem is that the continuation $k$ can be used to place $\dlet{\Binder{x}{\mathbb{N}}}{[-]}$ back on the stack. It is not clear, when the \texttt{Plus} AST is constructed and checked, that eager scope extrusion \textit{must} lead to lazy scope extrusion. 
...
The $\textbf{\texttt{check}}_M$ primitive allows $\Var{x}{\mathbb{N}}$ to temporarily extrude its scope. $\textbf{\texttt{check}}_M$ checks for scope extrusion, but turns a blind eye to some set of muted variables $M$. It thus suffices to add $\Var{x}{\mathbb{N}}$ to $M$. Inspection of the \textsc{Eff-Op} rule (\Cref{fig:corelang-opsem}, \Cpageref{fig:corelang-opsem}) shows that, when effects are performed, the variables which become unsafe (like $\Var{x}{\mathbb{N}}$) are added to the set of muted variables 
\[M \cup \projfvs{E_2}\]
...
\end{array}\]
Since the $\dlet{\Binder{x}{\mathbb{N}}}{[-]}$ frame is no longer on the stack, $\checkfv{\texttt{Plus}(\Var{x}{\mathbb{N}}, \texttt{Nat}(0))}$ would throw an error. However, performing the \textbf{\texttt{extrude}} operation additionally \textit{mutes} $\Var{x}{\mathbb{N}}$, and hence $\checkm{\texttt{Plus}(\Var{x}{\mathbb{N}}, \texttt{Nat}(\texttt{0}))}$  would not throw an error. 

When should a variable like $\Var{x}{\mathbb{N}}$ be \textit{unmuted}? When there \textbf{canot be} any bound continuations $k$ that could make place $\dlet{\Binder{x}{\mathbb{N}}}{[-]}$ on the stack. A safe approximation is the point where there cannot be \textit{any} bound continuations $k$. This point can be characterised as the maximal length $I$ of the stack $E$ that was never captured by the handling of an effect (\Cref{fig:unmute-variables-algo}). 
...
In the previous example, the transition from $\dlet{\Binder{z}{\mathbb{N}}}{\return{n}}$ unmutes variables. Therefore, $\Var{x}{\mathbb{N}}$ is still muted when the \texttt{App} constructor is checked ($\checkm{\App{f}{a}}$), but unmuted when we check the constructor of the outer lambda. 
This is the side-effect/side-condition in the \textsc{Sec-Dlt} rule (\Cref{fig:corelang-opsem}).

The best-effort scope extrusion check is thus simple: change all \textbf{\texttt{check}}s to $\textbf{\texttt{check}}_M$s. For example, 
{
  \footnotesize
\[
\begin{array}{rcl}
...
To augment the \sourceLang{} type system with a \textbf{simplified} version of \citeauthor{isoda-24}'s type system:
...
%   \end{enumerate}
% \end{lemma}
Proof of these the weakening lemma is by induction on the typing derivation.

Refined environment classifiers additionally require modification of the elaboration into \coreLang{} and the reduction of \coreLang{} terms:
...
If $\cdot \vdash ^{\gamma_\bot}_{\compilemode{}} e: T^0 \, ! \, \emptyset ; \emptyset $, and $\elaborate{e}_{\compilemode{}} = t$, \\
...
The logical relation on terms is defined as a least fixed point, following the definitions by \citet{plotkin-2025} and \citet{kuchta-2023}, where the well-foundedness of the definition is additionally justified. Defining the logical as a fixed point gives rise to the principle of \textsf{Scoped}-Induction
...
\[{\bind{\texttt{body}}{\elaborate{e}_{\compilemode}({\rho, \Binder{\alpha}{T_1^{\gamma'}} / x})}{\return{\Lam{\Binder{\alpha}{T_1^{\gamma'}}}{\texttt{body}}}}}\]
 ...
 By weakening, and the induction hypothesis, $\elaborate{e}_{\compilemode}({\rho, \Binder{\alpha}{T_1^{\gamma'}} / x}) \in \scoped[\Theta']{\effectType[\Delta; \xi]{T_2^{0}} (\gamma')}$, where $\Theta' = , \Theta, \gamma', \gamma \sqsubseteq \gamma'$. Applying \textsf{Scoped}-Induction on $\elaborate{e}_{\compilemode}({\rho, \Binder{\alpha}{T_1^{\gamma'}}})$:
\begin{enumerate}
