...
exhibits \textbf{\emph{lazy scope extrusion}} if all of the following hold:
  \begin{enumerate}
...
The \emph{\textbf{expressiveness}} of a dynamic scope extrusion check is defined as 
 \[\{ e \in \textsf{\emph{Safe}}\, \mid \langle \elaborate{e}^{\textbf{Check}};[-];\emptyset;\emptyset;\top \rangle \not\to^{*} \langle \err ; E ; U ; M ; I \rangle \} \]
\end{definition}
...
A configuration exhibits \textbf{eager scope extrusion} if it is improperly scoped (\Cref{subsubsection:eager-dynamic-check}, \Cpageref{subsubsection:eager-dynamic-check}). Notice that the definition drops the requirement that the configuration represents the result of compile-time execution, but is otherwise identical to \Cref{def:lazy-scope-extrusion}. 
