...
What is MacoCaml-style metaprogramming? I provide an answer in two steps. First, I motivate metaprogramming, by illustrating the challenge of writing code that is both fast and maintainable (\Cref{subsection:metaprogramming-motivation}). Second, I consider the design space of metaprogramming (\Cref{subsection:metaprogramming-design}), highlighting decisions made by MacoCaml designers.
...
The \mintinline{ocaml}{app: diff -> int} function evaluates elements of type \mintinline{ocaml}{diff}. 
...
We wish to write a function \mintinline{ocaml}{grad: diff -> float -> float} that computes the gradient of its first argument with respect to its second. The \mintinline{ocaml}{grad_main} function (\Cref{listing:ocaml-grad-main}) is one maintainable way to compute gradients: 
...
This approach is inefficient. If the function to be differentiated (\mintinline{ocaml}{f}) is known in advance, for example, \mintinline{ocaml}{f} $= 2(x^2 + 2x) + 4$, we could specialise \mintinline{ocaml}{grad} to a more efficient \mintinline{ocaml}{grad_spec} function (\Cref{listing:ocaml-grad-fast}), whose body is simply a hardcoded equation:
...
% Although \mintinline{ocaml}{grad_fast} only works for a single \mintinline{ocaml}{f}, it has eliminated the branching overhead, and enabled opportunities for prefetching. It is thus likely to be faster.

The \mintinline{ocaml}{grad} example illustrates the trade-off between maintainability and efficiency.\ \mintinline{ocaml}{grad_main} is maintainable because it parameterises over \mintinline{ocaml}{f}. More generally, abstraction centralises implementations, reducing maintenance costs. However, \mintinline{ocaml}{grad_spec} is more efficient because it is specialised to a specific \mintinline{ocaml}{f}. More generally, specialisation applies known arguments in advance, creating opportunities for optimisation. 
...
One answer, and the approach taken by \texttt{JAX} \citep{jax-grad-metaprogramming}, is metaprogramming, which gives users the ability to perform code-generation. Programmers may thus take matters into their own hands: manually generating optimised code when the compiler may not automatically do so for them. The \mintinline{ocaml}{grad} function in \texttt{JAX} uses metaprogramming for precisely this purpose \citep{jax-grad-metaprogramming}.

\subsubsection{The Mechanics of Metaprogramming}
Metaprogramming allows for code generation. Thus, it can be utilised to write a function, \mintinline{ocaml}{grad_gen}, which resembles \mintinline{ocaml}{grad_main} (inheriting its maintainability), but that generates a program which resembles \mintinline{ocaml}{grad_spec} (inheriting its efficiency). 

\Cref{listing:ocaml-pow-gen} presents the metaprogrammed \mintinline{ocaml}{grad_gen} function. On line 20, \mintinline{ocaml}{grad_gen} is used to generate \mintinline{ocaml}{(2 * (y+2)) * 2}, which resembles the body of \mintinline{ocaml}{grad_spec}. I will now explain the mechanics of generation.
...
\begin{enumerate}
  \item A type for code, with mechanisms for creating and manipulating values of this type. Expressions that return values of code type serve as code generators.
...
\begin{center}\mintinline{ocaml}{<<$(print "hello"; <<1>>) + 0 >>}\end{center} 

\noindent prints \mintinline{ocaml}{"hello"} and evaluates to \mintinline{ocaml}{<<1+0>>}.
...
\begin{center}\text{\mintinline{ocaml}{<<$(print "hello"; <<1>>) + 0>>}} \\ 
can be thought of as \\
 \text{\mintinline{ocaml}{Plus(print "hello"; Int(1), Int(0))}} \end{center}
...
To allow compile-time functions, like \mintinline{ocaml}{grad_gen}, to be re-used across multiple top-level splices, MacoCaml introduces the \mintinline{\macocamlLexer}{macro}.
