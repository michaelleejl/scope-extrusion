...
\begin{figure}[H]
  ...
  \node[] (normal-form) at ($(core-program.east) + (4cm, 0cm)$) {Return$($Nat$(0))$};
  \end{scope}
...
To re-iterate, I am considering the interaction between \textit{(homogenous, compile-time, two-stage)} \textbf{metaprogramming}, and an \textbf{effect system} \textit{(with deep handlers and multi-shot continuations)}. In this chapter, I describe a calculus, \calculusName{}, that combines both. There will be no attempt to mediate their interaction: specifically, it will not prevent scope extrusion. The aim will be to use \calculusName{} as a common language, on which one may layer different scope extrusion checks, evaluating them in a comparative fashion. 
...
Notice that we cannot quote values: we \textit{must} generate effectful programs. To my knowledge, \calculusName{} is the first such calculus. 

\subsection{Type System}
What is the type of $0$? Perhaps surprisingly, it is not $\mathbb{N}$. Since we are working with a two-stage system, we must be careful to disambiguate between run-time naturals and compile-time naturals, since these are not interchangeable. We thus annotate each type with a level: $\mathbb{N}^0$ for run-time naturals, and $\mathbb{N}^{-1}$ for compile-time naturals. More generally, types are stratified into two levels: $T^0$ (run-time) and $T^{-1}$ (compile-time). 
