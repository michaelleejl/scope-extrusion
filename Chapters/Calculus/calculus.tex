...
Given a term of the form $\checkfv{n}$ in some evaluation context $E$, where $n$ is an AST, $\textbf{\texttt{check}} \, n$ succeeds ($n$ is ``properly scoped'' in $E$) if and only if the free \texttt{Var}s of $n$, written $\freevars{n}$, have all been declared safe in $E$, $\freevars{n} \subseteq \projfvs{E}$. 
...
The \coreLang{} typing rules (\Cref{fig:core-typing-rules}) are extremely straightforward: for example, $\Binder{\alpha}{R}$ is an \textsf{FParam} of type $R$, and $\Var{\alpha}{R}$ is an \textsf{AST} of type $R$. $\gensym{R}$ is a term that produces \textsf{FParam}s of type $R$, and $\Lam{n_1}{n_2}$ is well-typed if $n_1$ is an \textsf{FParam} and $n_2$ an AST of some run-time computation type. 
