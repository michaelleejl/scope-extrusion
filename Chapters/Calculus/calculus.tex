...
\sourceLang{} extends \efflang{} with quotes and splices. Recall that \efflang{}, following a fine-grained call-by-value approach, divides terms into two syntactic categories, values $v$ and computations $c$. \sourceLang{} is similar, dividing terms into values $v$ and expressions $e$ (\Cref{fig:source-syntax})
...
\begin{tikzpicture}
  ...
  \begin{scope}[on background layer, font=\scriptsize\bfseries, text=comment, align=left]
    \draw[draw=comment, line width = 0.3mm] ($(example.north east) + (-2.4cm, 0.6cm)$) circle[radius=1.4mm] node (annote-1) {\tiny{1}};
    \node[anchor = east, align=right] (annote-1-text) at ($(annote-1.west) + (0.1cm, -0.15cm)$) {Compile-time \\function} ;
    \draw[draw=comment, line width =0.3mm] ($(example.north east) + (-1.9cm, 0cm)$) |- (annote-1.east);
    ...
    \draw[draw=comment, line width = 0.3mm] ($(example.south west) + (2.8cm, -0.4cm)$) circle[radius=1.4mm] node (annote-3) {\tiny{3}};
    ...
    \draw[draw=comment, line width =0.3mm] ($(example.south west) + (2.3cm, 0cm)$) |- (annote-3.west);

    \draw[draw=comment, line width = 0.3mm] ($(example.south east) + (-6.7cm, -0.4cm)$) circle[radius=1.4mm] node (annote-4) {\tiny{4}};
    ...
    \draw[draw=comment, line width =0.3mm]  ($(example.south east) + (-7.2cm, 0cm)$) |- (annote-4.west);
  \end{scope}
...
The typing rules for scope extrusion checks are even more straightforward: they are effectively invisible to the type system.
\begin{figure}
  \begin{core-desc}
...
Is this true in general? Do \sourceLang{} typing derivations always elaborate into \coreLang{} typing derivations? More generally, what properties can we claim (and not claim) about the calculus as defined? To answer these questions, we prove certain metatheoretic results.
