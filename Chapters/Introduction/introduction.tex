...
Since \mintinline{ocaml}{architecture} and \mintinline{ocaml}{vector_extensions} are known at compile-time, they may be eliminated by an optimising compiler. If the compiler does not perform this optimisation, programmers might have to rely on pre-processing directives, or duplicate their code, increasing maintenance costs. With metaprogramming, however, they may write: 
\begin{macocaml}
...
This thesis solves both these problems. In \Cref{chapter:calculus}, I design a common language for encoding and evaluating different solutions. In \Cref{chapter:scope-extrusion}, I use the designed language to formally describe and evaluate various checks. This process led to a novel Best-Effort dynamic check (\Cref{section:best-effort-check}), which I will argue lands in a goldilocks zone, and should be adopted by MacoCaml.
